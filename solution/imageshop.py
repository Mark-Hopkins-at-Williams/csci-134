...
def rotate_left(image):
    ...
    """
    result = zeros(len(image[0]), len(image))
    for row in range(len(image)):
        for col in range(len(image[0])):
            result[col][row] = image[row][len(image[0])-col-1]
    return result
...
def convolve(image, kernel):
    ...
    Your extra doctests here
    """
    result = zeros(len(image)-2, len(image[0])-2)
    for row in range(1, len(image)-1):
        for col in range(1, len(image[0])-1):
            convolved = 0
            for kernel_row in range(3):
                for kernel_col in range(3):
                    convolved += image[row+kernel_row-1][col+kernel_col-1] * kernel[kernel_row][kernel_col]
            result[row-1][col-1] = convolved            
    return result
